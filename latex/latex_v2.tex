...
% 实验使用一个标准的SVM分类器对PET和CT图像进行分类,使用hinge loss，这是SVM分类的标准损失函数，适用于处理分类问题中的间隔最大化。，模型采用随机梯度下降（SGD）作为优化算法。lr（学习率）设置为0.001，这是一个相对较低的学习率，可以帮助模型在训练过程中稳定地逼近全局最优解。weight_decay参数设置为0.01，用于实现L2正则化，有助于减轻模型过拟合的问题，增强模型的泛化能力。实验记录包括多个训练周期，每个周期中计算了损失值、正确分类的样本数、样本总数和分类准确率。数据采集自实验的连续批处理，每个批次包含不同数量的图像样本。实验结果显示，SVM分类器在初期训练阶段的准确率为50\%。SVM在训练时的表现如下图所示。
