...
class GAN_base(ABC):
    def __init__(self, in_channels=3, out_channels=3):
        """
        初始化 GAN_base 类，包含模型名称、生成器和判别器。
        """
        self.name = "GAN_base"
        self.generator = Generator_base(in_channels, out_channels)
        self.discriminator = Discriminator_base(in_channels)
        
        self.cuda = torch.cuda.is_available()
        self.device_ids = list(range(torch.cuda.device_count()))  # 获取所有可用GPU的索引
        self.available_device_ids = None
        if self.cuda :
            self.available_device_ids = [0]
        
    
    def calculate_loss(self, pred_y, truth_y, x):
        # Discriminator loss
